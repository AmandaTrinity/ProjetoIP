...
class Botao:
    def __init__(self, imagem_normal, imagem_hover, pos_x, pos_y):
        ...
        # A imagem 'atual' começa como a normal
        self.imagem_atual = self.imagem_normal
        
        # O rect é criado a partir da imagem atual para ter o tamanho correto
    ...
    def atualizar_feedback(self, posicao_mouse):
        # Atualiza a imagem do botão (normal ou hover) com base na posição do mouse.
        if self.rect.collidepoint(posicao_mouse):
            # Se o rato está em cima, usa a imagem de hover
            self.imagem_atual = self.imagem_hover
        else:
            # Se o rato está fora, usa a imagem normal
            self.imagem_atual = self.imagem_normal